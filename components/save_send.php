...
if(isset($_POST['send'])){
   if($user_id != ''){
      ...
      $verify_request = $conn->prepare("SELECT * FROM `request` WHERE property_id = ? AND sender = ? AND receiver = ?");
      $verify_request->execute([$property_id, $user_id, $receiver]);

      if(($verify_request->rowCount() > 0)){
         $warning_msg[] = 'request sent already!';
      }else{
         $send_request = $conn->prepare("INSERT INTO `request`(id, property_id, sender, receiver) VALUES(?,?,?,?)");
